package kr.or.yi.coffee_exam.ui;
...
public class RankListPanel extends JPanel {
	...
	private Object[] getSaleRow(Sale sale) {
		...
		return new Object[] {
				...
				String.format("%,d", sale.getSupplyPrice()),	//공급가액
				String.format("%,d", sale.getAddTax()), //부가세액
				String.format("%,d", sale.getSalePrice()), //판매금액
				sale.getMarginRate() + "%", //마진율
				String.format("%,d", sale.getMarginPrice())	//마진액
		};
	}
	...
	public Object[] getTotal() {
		...
		for(Sale s : list) {
			totalSupplyPrice += s.getSupplyPrice();
			totalAddTax += s.getAddTax();
			totalSalePrice += s.getSalePrice();
			totalMarginPrice += s.getMarginPrice();
